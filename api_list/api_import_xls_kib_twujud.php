...
$aColumns = array('temp_twujud_ID','temp_twujud_ID','kodeKelompok','uraian','kodeLokasi','Jumlah','NilaiPerolehan','temp_twujud_ID');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "temp_twujud_ID";
...
$dataSESSION = $RETRIEVE_PEROLEHAN->get_tmpData($dataParam,'tmp_twujud'); 
//pr($dataSESSION);
//exit;
...
$data_post=$PENGHAPUSAN->apl_userasetlistHPS("XLSIMPWJD");
...
if($POST){
  ...
  $newlist = array();
      ...
      foreach ($dataSESSION as $keySESSION => $valueSESSION) {
        ...
        if(!in_array($valueSESSION['temp_twujud_ID'], $newlist)){
          // echo "stringnot";
          $data[]=$valueSESSION;
...
foreach ($data as $key => $value)
               ...
               $checkbox="<input {$value['style']} type=\"{$value['other']}\" {$value['checked']} id=\"check_{$no}\" class=\"icheck-input\" name=\"aset[]\" 
                      value=\"{$value['temp_twujud_ID']}|{$value['NilaiTotal']}\" onchange=\"return AreAnyCheckboxesChecked();\">";
               $row[]=$no;
               $row[]=$checkbox;
