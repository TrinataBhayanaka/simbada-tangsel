...
$aColumns = array('temp_kdp_ID','temp_kdp_ID','kodeKelompok','uraian','kodeLokasi','Jumlah','NilaiPerolehan','temp_kdp_ID');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "temp_kdp_ID";
...
$dataSESSION = $RETRIEVE_PEROLEHAN->get_tmpData($dataParam,'tmp_kdp'); 
//pr($dataSESSION);
//exit;
...
$data_post=$PENGHAPUSAN->apl_userasetlistHPS("XLSIMPKDP");
...
if($POST){
  ...
  $newlist = array();
      ...
      foreach ($dataSESSION as $keySESSION => $valueSESSION) {
        ...
        if(!in_array($valueSESSION['temp_kdp_ID'], $newlist)){
          // echo "stringnot";
          $data[]=$valueSESSION;
...
foreach ($data as $key => $value)
               ...
               $checkbox="<input {$value['style']} type=\"{$value['other']}\" {$value['checked']} id=\"check_{$no}\" class=\"icheck-input\" name=\"aset[]\" 
                      value=\"{$value['temp_kdp_ID']}|{$value['NilaiTotal']}\" onchange=\"return AreAnyCheckboxesChecked();\">";
               $row[]=$no;
               $row[]=$checkbox;
